...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        ################################################################################
        # Test set classification accuracy: 0.7418000102043152
        ...
        ## 
        # kl_y = D_KL ( q(y | x) || p(y) )
        log_p_y = torch.log(torch.ones(self.y_dim) / self.y_dim)
        ...
        # kl_z = E[ D_KL (q(z | x, y)) || p(z) ]
        kl_z = ut.kl_normal(m, v, self.z_prior_m , self.z_prior_v) 
        kl_z = kl_z.reshape(self.y_dim, -1).T
        kl_z = torch.sum(kl_z * y_prob, dim=1).mean()
 
        # rec = E[ -log p(x | z,y) ]
        rec = -ut.log_bernoulli_with_logits(x, logits)
        rec = rec.reshape(self.y_dim, -1).T
        rec = torch.sum(rec * y_prob, dim=1).mean()

        nelbo = kl_z + kl_y + rec

        # Test set classification accuracy: 0.91129999828338623
        ################################################################################
